...
export default function DiskonOtomatis() {
  ...
  // 1. Ambil Data Cabang dan Kategori saat pertama kali dimuat
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchRules = async () => {
      setIsLoading(true);
      try {
        const response = await axiosInstance.get(`/owner/diskon-rules?cabang_id=${selectedCabang}`);
        setRules(response.data.data || []);
      } catch (error) {
        console.error("Gagal mengambil aturan diskon:", error);
      } finally {
        setIsLoading(false);
      }
    };
    ...
    const handleGlobalSync = () => fetchRules();
  ...
  const handleCreateRule = async () => {
    if (!pemicu || !diskon || !selectedCabang) return;
    setIsSaving(true);
    ...
    try {
      const payload = {
        ...
        cabang_id: selectedCabang // Sisipkan ID Cabang ke payload
      };
    ...
    <div className="animate-fadeIn pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm h-12">
            <span className="pl-3 text-sm font-bold text-slate-700">Cabang:</span>
            <select 
              value={selectedCabang}
              onChange={(e) => setSelectedCabang(e.target.value)}
              className="pr-8 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-semibold text-[#0B3B91] focus:outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer"
          ...
          {/* Main Toggle */}
          <div className="bg-white rounded-2xl border border-gray-100 px-5 py-2 shadow-sm flex items-center gap-4 h-12">
            <div>
              <p className="text-sm font-semibold text-gray-900 leading-tight">Sakelar Utama</p>
            </div>
            <button
              onClick={() => setMainToggle(!mainToggle)}
              className={`relative w-12 h-6 rounded-full transition-all duration-300 ${mainToggle ? 'bg-blue-600' : 'bg-gray-300'}`}
            >
              <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all duration-300 ${mainToggle ? 'left-6.5' : 'left-0.5'}`} />
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm h-fit">
          ...
          <div className="space-y-5">
            ...
            <button 
              ...
              disabled={isSaving || !pemicu || !diskon || !selectedCabang}
        ...
        <div>
          ...
          {isLoading ? (
             <div className="bg-white rounded-xl border border-gray-100 p-8 shadow-sm text-center">
               <p className="text-sm text-gray-400 font-medium animate-pulse">Menarik data aturan diskon...</p>
             </div>
          ) : (
            <div className="space-y-4">
              {rules.length > 0 ? rules.map((rule) => (
                ...
                <div className="bg-white rounded-xl border border-gray-100 p-8 shadow-sm text-center">
                  <p className="text-sm text-gray-500 font-medium">Belum ada aturan diskon untuk cabang ini.</p>
                </div>
              )}
            </div>
          )}
