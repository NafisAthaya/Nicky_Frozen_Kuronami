...
export default function Transaksi() {
  ...
  // 1. Ambil Pengaturan Struk & Daftar Cabang saat komponen dimuat
  // 1. Ambil Pengaturan Struk & Daftar Cabang saat komponen dimuat
  useEffect(() => {
    const fetchInitialData = async () => {
      // --- TARIK PENGATURAN STRUK ---
      try {
        // Jika route ini 404, cek kembali routes/api.php kamu
        const resSettings = await axiosInstance.get('/owner/pengaturan-toko');
        if (resSettings.data && resSettings.data.data) {
          setStrukSettings(resSettings.data.data);
        }
      } catch (err) {
        console.error("Gagal mengambil pengaturan struk (diabaikan):", err);
      }
  ...
  // 2. Ambil Data Transaksi (Dinamic berdasarkan selectedCabang)
  useEffect(() => {
    const fetchTransactions = async () => {
      setIsLoading(true);
      try {
        const url = selectedCabang ? `/owner/transaksi?cabang_id=${selectedCabang}` : '/owner/transaksi';
        ...
        // Mapping data dari Backend agar sesuai dengan struktur Modal Struk
    ...
    <div className="animate-fadeIn relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 bg-white p-2 rounded-xl border border-slate-200 shadow-sm">
          <span className="pl-2 text-sm font-bold text-slate-700">Cabang:</span>
          <select 
            value={selectedCabang}
            onChange={(e) => setSelectedCabang(e.target.value)}
            className="pr-8 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-sm font-semibold text-[#0B3B91] focus:outline-none focus:ring-2 focus:ring-blue-100 cursor-pointer"
          >
      ...
      {/* Transaction Cards */}
      {isLoading ? (
        <div className="p-8 text-center text-gray-500 animate-pulse bg-white rounded-xl border border-gray-100">
          Menyiapkan daftar transaksi hari ini...
        </div>
      ) : (
        <div className="space-y-4">
          {filteredTransactions.length > 0 ? (
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{trx.cabangName}</span>
                      </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-12 shadow-sm text-center">
              ...
              <p className="text-gray-500 font-medium">Belum ada transaksi di rentang waktu/cabang ini.</p>
            </div>
          )}
        </div>
      )}
            ...
            <div className="flex-1 bg-white rounded-3xl p-6 shadow-md border border-slate-100 relative flex flex-col justify-between">
              <div>
                <div className="text-center mb-4">
                  ...
                  <p className="text-[10px] font-bold text-slate-500 mt-1 uppercase tracking-wider">{selectedTrx.cabangName}</p>
